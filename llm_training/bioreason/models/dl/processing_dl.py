...
class DLProcessor(ProcessorMixin):
    ...
    def __init__(
        ...
        # ── 改动1：dna_tokenizer=None 时跳过父类类型检查 ──────────────────
    ...
    def tokenize_dna_sequences(
        ...
        # ── 改动2：纯LLM模式 dna_tokenizer=None 时直接返回空 ─────────────
    ...
    def __call__(
        ...
        **kwargs: Unpack[DLProcessorKwargs],
        ...
        if batch_dna_sequences is not None:
            ...
            # ── 改动3：根据是否有 dna_tokenized 分两种模式处理 ───────────
            if dna_processing_result['dna_tokenized'] is not None:
                # DNA-LLM模式：把文本中的 dna_token 替换为对应数量的占位符
                index = 0
                for i in range(len(text)):
                    while self.dna_token in text[i]:
                        # ?? FIX: 强制转换为 int，防止浮点数导致的 TypeError
                ...
                # 纯LLM模式：清除文本中所有残留的 dna_token（如有）
                for i in range(len(text)):
